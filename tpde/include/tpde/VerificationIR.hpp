...
struct VerificationIR {
  ...
  struct Allocation {
    ...
    Allocation(i32 off, u32 part = 0)
        : is_stack(true), part_idx(part), stack_off(off) {}

    bool operator==(const Allocation &other) const {
      if (is_stack != other.is_stack || part_idx != other.part_idx) {
        return false;
      }
    ...
    std::string format() const {
      if (is_stack) {
        return part_idx > 0 ? std::format("[sp+{}]:{}", stack_off, part_idx)
                            : std::format("[sp+{}]", stack_off);
      } else {
        return part_idx > 0
                   ? std::format("r{}:{}", static_cast<u32>(reg.id()), part_idx)
                   : std::format("r{}", static_cast<u32>(reg.id()));
      }
    }
  };
  ...
  struct Operand {
    ...
    std::string format_with_current_alloc(
        const std::map<std::pair<ValLocalIdx, u32>, Allocation> &current_allocs)
        const {
      auto key = std::make_pair(val_idx, alloc.part_idx);
      auto it = current_allocs.find(key);
      const Allocation &alloc =
          it != current_allocs.end() ? it->second : this->alloc;
      return std::format("%v{}@{}", static_cast<u32>(val_idx), alloc.format());
    }
  };
  ...
  struct InstOp : Formattable {
    ValLocalIdx inst_id; // Identifier for the instruction (uses result val_idx
                         // if available, otherwise marker)
    util::SmallVector<Operand, 4> uses; // Input operands
    util::SmallVector<Operand, 2> defs; // Output operands
    BlockIndex branch_target = static_cast<BlockIndex>(
        ~0u); // Target block for branches (invalid if not a branch)
    util::SmallVector<PhiIncoming, 4>
        phi_incomings;     // Incoming values for PHI nodes (empty if not a PHI)
    std::string jump_type; // Jump type for branches (e.g., "jmp", "je", "jne")
    bool is_argument = false; // True if this is a function argument

    InstOp(ValLocalIdx id,
           util::SmallVector<Operand, 4> u,
           util::SmallVector<Operand, 2> d)
        : inst_id(id),
          uses(std::move(u)),
          defs(std::move(d)),
          branch_target(static_cast<BlockIndex>(~0u)),
          is_argument(false) {}
    ...
    std::string format(const std::map<std::pair<ValLocalIdx, u32>, Allocation>
      ...
      if (is_argument) {
        ...
        std::string target_str =
            (static_cast<u32>(branch_target) & 0x80000000u)
                ? std::format("split_b{}",
                              static_cast<u32>(branch_target) & 0x7FFFFFFFu)
        ...
        if (uses_str.empty() && defs_str.empty()) {
          return "";
        }
        if (uses_str.empty()) {
          return std::format("  op defs={}\n", defs_str);
  ...
  struct Edit : Formattable {
    ...
    std::string format() const override {
      ...
      if (kind == EditKind::RegMove) {
        ...
        auto format_alloc_no_part = [](const Allocation &alloc) {
          if (alloc.is_stack) {
            return std::format("[sp+{}]", alloc.stack_off);
          }
          return std::format("r{}", static_cast<u32>(alloc.reg.id()));
        };
        std::string value_str =
            from.part_idx > 0 ? std::format("%v{}:{}",
                                            static_cast<u32>(val_idx),
                                            from.part_idx)
                              : std::format("%v{}", static_cast<u32>(val_idx));
        return std::format("  edit {} {} -> {} {} {}b\n",
                           kind_str,
                           format_alloc_no_part(from),
                           format_alloc_no_part(to),
                           value_str,
                           size);
      }
    }
  ...
  /// Track condition value for current branch (set before
  ...
  /// Get function name
  const std::string &get_func_name() const noexcept { return func_name; }

  BlockIndex begin_branch(BlockIndex from_block,
                          BlockIndex to_block,
  ...
  /// Emit general move even during active compilation
  void emit_active_reg_move(Reg src, Reg dst, u32 size) noexcept {
    emit_reg_move_impl(src, dst, size);
  }
...
public:
  void emit_arg(BlockIndex entry_block_idx,
                ValLocalIdx val_idx,
                u32 part_idx,
                Allocation alloc) noexcept {
  ...
  void vir_emit_phi(ValLocalIdx phi_idx,
                    u32 part_idx,
                    Allocation phi_alloc,
  ...
  void capture_branch(const char *jump_type,
    ...
    BlockIndex current_block =
        current_split_block != static_cast<BlockIndex>(~0u)
            ? current_split_block
            : current_block_idx;
    BlockIndex edit_block = begin_branch(current_block, target_block, is_split);
    ...
    // Use a marker val_idx based on the block index (will never conflict with
    // real values)
    ValLocalIdx branch_id =
        static_cast<ValLocalIdx>(static_cast<u32>(current_block) | 0x40000000u);
    InstOp inst_op{
        branch_id,
        std::move(uses),
