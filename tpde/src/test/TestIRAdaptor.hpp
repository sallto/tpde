...
struct TestIRAdaptor {
  ...
  [[nodiscard]] bool inst_has_call(IRInstRef inst) const {
    const auto &value = ir->values[static_cast<u32>(inst)];
    return value.op == TestIR::Value::Op::call;
  }
  ...
  struct ValueParts {
    ...
    static RegBank reg_bank(u32) { return RegBank{0}; }
  };
