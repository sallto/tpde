...
int main(int argc, char *argv[]) {
    ...
    args::Flag print_phi_webs(parser,
                              "print_phi_webs",
                              "Print PHI coalescing webs",
                              {"print-phi-webs"});
...
#ifdef TPDE_LOGGING
    {
        ...
        switch (log_level.Get()) {
            case 0: level = spdlog::level::off;
                break;
            case 1: level = spdlog::level::err;
                break;
            case 2: level = spdlog::level::warn;
                break;
            case 3: level = spdlog::level::info;
                break;
            case 4: level = spdlog::level::debug;
                break;
            default:
                assert(level >= 5);
    ...
    if (run_until.Get() == RunTestUntil::only_analyzer) {
        ...
        for (auto func: adaptor.funcs()) {
            ...
            if (print_phi_webs) {
                std::cout << "Phi Webs for " << adaptor.func_link_name(func) << "\n";
                analyzer.print_phi_webs(std::cout);
                std::cout << "End Phi Webs\n";
            }
    ...
    switch (arch.Get()) {
        case Arch::x64: compile_fn = &test::compile_ir_x64;
            break;
        case Arch::a64: compile_fn = &test::compile_ir_arm64;
            break;
